...
# --- 3. KONEKSI KE GOOGLE AI STUDIO ---
try:
    genai.configure(api_key=st.secrets["GOOGLE_API_KEY"])
except KeyError:
    st.error("⚠️ API Key GOOGLE_API_KEY belum disetting di file secrets.toml.")
    st.stop()
    ...
    with st.chat_message("assistant"):
        with st.spinner("⏳ Antosan sakedap, asisten sedang mengetik..."):
            try:
                ...
                st.error(f"Hapunten, terjadi kesalahan teknis (Google AI): {e}")
                st.stop()
