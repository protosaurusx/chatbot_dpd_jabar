...
# --- 3. KONEKSI KE GROQ ---
try:
    # Menggunakan library Groq
    client = Groq(api_key=st.secrets["GROQ_API_KEY"])
except KeyError:
    st.error("⚠️ API Key Groq belum disetting di file secrets.toml.")
    st.stop()
    ...
    with st.chat_message("assistant"):
        with st.spinner("⏳ Antosan sakedap, asisten sedang mengetik..."):
            try:
                ...
                st.error(f"Hapunten, terjadi kesalahan teknis (Groq): {e}")
                st.stop()
